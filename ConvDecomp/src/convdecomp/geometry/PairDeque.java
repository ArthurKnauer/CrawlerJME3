...
final public class PairDeque {
	...
	public void pushNarrow(int i, int j) { // no nesting--> aF<i & bF<j
		if ((!empty()) && (i <= aF()))
			return; // don't push wider
		while ((!empty()) && (bF() >= j))
			pop(); // pop until narrower: bF<j
		push(i, j);
	}
	...
	private PairDeque merge(PairDeque pd) { // untested
		...
		while ((!emptyB()) && !pd.emptyB())
			if (aB() < pd.aB()) {
				tmp.pushNarrow(aB(), bB());
				popB();
			} else {
				tmp.pushNarrow(pd.aB(), pd.bB());
				pd.popB();
			}
		while (!emptyB()) {
			tmp.pushNarrow(aB(), bB());

...
/**
 * This is intended to be a replacement for the internal bullet character class. A RigidBody with cylinder collision
 * shape is used and its velocity is set continuously, a ray test is used to check if the character is on the ground.
 *
 * The character keeps his own local coordinate system which adapts based on the gravity working on the character so the
 * character will always stand upright.
 *
 * Forces in the local x/z plane are dampened while those in the local y direction are applied fully (e.g. jumping,
 * falling).
 *
 * @author normenhansen
 */
public class CustomCharacterControl extends AbstractPhysicsControl implements PhysicsTickListener {
	...
	protected static final Logger logger = Logger.getLogger(CustomCharacterControl.class.getName());
	 ...
	 *
	 * @param factor The factor by which the height should be multiplied when ducking
	 */
	 ...
	 * alinged with Z axis.
	 *
	 * @param vec The new forward vector, has to be perpendicular to the current gravity vector!
	 */
	 ...
	 * to the up vector.
	 *
	 * @param rotation The rotation to set the result on or null to create a new Quaternion, this will be set to the new
	...
	public Control cloneForSpatial(Spatial spatial) {
		CustomCharacterControl control = new CustomCharacterControl(radius, height, mass);
